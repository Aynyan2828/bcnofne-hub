...
各セクションの手前に **海と空だけの一画面** を挟んだ。
  航海（海だけ）→ 到着（港の名前が大きく出る）→ 中身（カード）
という呼吸ができて、「着いた」が分かるようになる。

### Added
- `src/components/HarborGate.astro` — 港の入口（62vh／PCは56vh）。
  `ARRIVING` → 港の英字名が奥から手前に寄ってくる → 日本語の一行 →
  水平線の細い線が左右に開く。
- `Section` に `harborLabel`（日本語の一行）。APPS=アプリの港 / RADIO=聴く港 /
  MUSIC=音の港 / SIGNAL=便りの港。

### 実機で直したこと
- **小さい文字が海の柄に負けて読めんかった**（ARRIVING と日本語の一行）。
  白い光の縁を回して、さらに文字のうしろにだけ淡い光だまりを敷いた。
  港の入口は紙だまりの外やけん、これが無いと海に食われる。
- **「次の港へ」が画面の真ん中下に居座って本文に重なっとった**。左下へ移した
  （右下は AYN ナビが居る）。小さくして、居ることは分かる程度に。

### 守ったこと
- **カードの追加しやすさは壊しとらん。** 港は「入れ物」の話で、中身のカードは
  今までどおり `src/content/*.md` が正本。`.md` を1枚足せばカードが1枚増える。
  港を増やす時も `src/lib/voyage.ts` に1行 ＋ Section に `harbor` を渡すだけ。

## 2026-09-25 (8) — 照り返しを見える濃さに直す ＋ 港の間隔を広げる

### Fixed
- **照り返しが見えとらんかった。** 最初 `soft-light` の 0.42 で出したけど、
  白い紙の上ではほぼ消える。読みやすさを気にして振り方が保守的すぎた。
  → `overlay` の 0.9 に。overlay は**明暗を保ったまま色だけ乗る**けん、
    字は黒いまま・紙だけ色が寄る＝読めるのに通過が分かる。
  → 帯の幅も絞って輪郭を持たせた（ぼんやり広げると何も起きとらんように見える）。

### Changed
- **港と港の間隔を広げた**（padding-block を clamp(2.8rem,7vw,4.5rem) →
  clamp(5.5rem,13vw,9rem)）。ここが「航海しとる時間」になる＝海と空だけの帯が
  できて、次の港に着いた感じが出る。

## 2026-09-25 (7) — 空の高みから海面へ降りていく（カメラ）＋ 遠景を足して厚くする

みさととの2本目の録画から。あれは**空から町へ降りて絵の中に入っていく**体験やった。
こっちは海やけん「**空の高みから海面へ降りていく**」に翻訳した。

### Changed（OceanField のカメラ）
- ページ全体の進み(0→1)で**視点の高さが下がる**:
    高い … 水平線が低く、海が広く、波は小さく遠い（俯瞰）
    低い … 水平線が上がって、波が大きく迫る（海面すれすれ）
  水平線 H*0.34 → H*0.56、波の大きさ 0.72倍 → 1.32倍。
  横に流すだけの視差やなく**視点そのものが動く**けん「世界に入っていく」感じが出る。
  実測: 進み0% で水平線 37%、90% で 55%（設計どおり降りとる）。

### Added（第2バッチのパーツ・世界を厚く）
- `moon` `cape_lighthouse` `isle_a` `isle_b` `mist_loop` の5枚（計12枚・約1.2MB）
- **遠景の目印**を水平線の向こうに置いた。月 → 島影 → 灯台のある岬 がゆっくり
  左へ流れる。ここに何か在るだけで「どこへ向かっとるか」が分かる。
- **霧**は航海が深まるほど濃くなる（進み 0.35 以降）＝深夜の海。
- **「次の港へ」の矢印**（みさととの下向き矢印と同じ役目）。最後の港では
  「港へもどる」に変わる。スクロール中は薄れて邪魔せん。

### 検証の口
- `window.__oceanDraw(t)` を追加。裏タブ・描画エンジンでは rAF が来んけん、
  スクロールを変えても絵が描き直されん＝目でも測定でも確かめられんかった。
  手で1コマ描ける口を置いて、カメラの降下を数値で確認できるようにした。

## 2026-09-25 (6) — 文字とカードの上を、海の照り返しが撫でていく

マスターが whatever.co の画面録画で見せてくれた「**色の面が本文の上を通り過ぎて、
その下の文字の色が変わる**」を、BCNOFNe の世界に翻訳した。
あっちは原色のベタ面やけど、こっちは**水面の照り返し**（青〜紫〜桃）にする。

### Added
- `src/components/LightWash.astro` — 照り返しの見た目（CSS のみ）。
  - セクションごとに薄い帯を1枚。スクロール量で斜めに動く。
  - `mix-blend-mode: soft-light` で**本文の上**を通す＝通過したところだけ字の色が寄る。
    下に置くと紙だけ染めて字は変わらん。ただし濃さは 0.42 に抑えた（読みやすさが最優先）。
  - 見出しの eyebrow（APPS / MUSIC …）の上に**小さな色チップ**。whatever.co のあれの水彩版。

### 設計メモ
- **帯を差し込むのは OceanField 側**。最初 LightWash の script で差し込んだら、
  OceanField が帯を集める時にまだ存在せんで動かんかった（別コンポーネント同士やと
  script の実行順は保証されん）。差し込みと収集は同じ場所でやる。
- 動かすのも OceanField の描画ループから（ループを2本持たん）。
  画面に入っとるセクションの帯だけ触る。

## 2026-09-25 (5) — 波を「手前へ迫らせて」立体的に進ませる

マスターの指摘「立体的に進んでいるように見えるようにならん？横スクロールはちょっと」。
そのとおりで、帯を横に流すだけやと**景色が横に動くだけ**で、前へ進んどるようには
見えん。作り直した。

### Changed（OceanField の波）
- 波を **水平線から手前へ迫らせる** 方式に。各行に奥行き z を持たせて、
  スクロールで z を減らす＝自分に向かってくる。
    z=1 … 水平線のすぐ向こう ／ z=0 … 目の前（画面の下）
  近いほど **下に来て・大きくなって・速く流れる**（遠近法）。
  画面の下を通り過ぎた行は水平線の向こうへ戻して使い回す（9行で回す）。
- 水平線際でふわっと湧いて手前で薄れる＝唐突に出入りせん。
- 水平線を H*0.60 → **H*0.46** に上げた（奥行きを取る余地を作るため）。
- 小舟は「ある深さ(k=0.42)」に居る扱い。その深さの波を跨ぐ手前で描くけん、
  **手前の波に隠れて波間に沈む**。
- 雲も手前に流して上へ抜けさせた（＝自分が進んどる）。
- 帯のタイル枚数に上限(40)。遠い帯は1枚が小さくなって枚数が増えるけん。

## 2026-09-25 (4) — 背景を水彩パーツの合成に作り直し（みさとと方式）

procedural に点と線を描いとった背景を捨てて、**GPTプラグインで描いた水彩パーツを
canvas で合成する**方式に作り直した。「弱い」の原因は絵が無かったこと。

### Added
- `public/world/*.webp` — 水彩パーツ7枚（合計 904KB）:
  `cloud_loop` `wave_far` `wave_mid` `wave_near` `ship_hull` `ship_sail` `ship_lantern`
- `ayn-sleep-radio/scripts/gen_world_assets.py` — 生成＋白抜き＋縮小＋WebP化。

### Changed（OceanField）
- **部品を別々に動かす**: 波で船体が傾く → 帆がワンテンポ遅れて揺れる →
  ランタンだけ点滅する。みさととが `kagura_head` / `kagura_hand_left` と
  部品に割っとったのと同じ理屈。1枚絵やと全部同じに動いて安っぽい。
- **帯は鏡張りで無限ループ**: 元絵とその左右反転を交互に並べると、どの継ぎ目でも
  端の絵柄が一致する。GPT に継ぎ目の合う絵を描かせるのは無理やけん、こっちで作る。
- 奥行き＝流れる速さ。雲 0.05 / 遠い波 0.08 / 中の波 0.16 / 手前の波 0.28（対スクロール）。
- 小舟を**中の波と手前の波で挟んで**描く＝波間に沈んで見える。
- 絵が1枚も読めんでも粒子だけで成立する（絵に依存して真っ白にならん）。

### 生成で踏んだ地雷（scripts 側にコメントで残した）
- **単体の人工物を白地に頼むと写実に流れる**。船体は木目と釘まで描いたニス塗りの
  ボート、帆は縫い目とロープ入りの作図イラストが出た。波や雲のような「情景の帯」は
  素直に水彩で出るのに。→「輪郭線を引かん」「写実・図鑑・製品写真にせん」
  「描き込まん」を明示して締める。
- **背景に紙のテクスチャを描かれると輝度で抜けん**（夜空の上に四角い紙の跡が出る）。
  →「背景はのっぺりした真っ白に、紙の質感は描かんで」。
- **紙と同じ明るさの絵は抜きようが無い**（生成りの白い帆）。→ 色を指定して分離させる
  （月あかりの青灰色）。
- 白抜きの閾値は3回作り直した: 固定値 → 四辺の縁から測る（波は端まで描いてあるけん
  誤判定して絵が消えた）→ **四隅の小さな正方形だけ**で安定。
- 失敗検知（「紙が見つからん」「抜いたら中身がほぼ消えた」）を入れた。実際これが
  ランタンを捕まえた。ただし最初は**戻り値を見とらんくて、弾いた絵まで「できた」と
  報告しとった**（報告が嘘をつくのがいちばん困る）→ 直した。

## 2026-09-25 (3) — カードが海面から浮かび上がる ＋ 水彩パーツの生成を開始

### Changed
- **カードの出現を「湧く」から「海面から浮かび上がる」へ。**
  情報の海を進んどるんやけん、その方が理屈が合う。
  28px 下から、ぼやけ(blur 5px)＋わずかに小さい(0.965) → 上がりきると くっきり原寸。
  棚（横スクロール）の中身も1枚ずつ順に浮上する（70ms ずつ遅らせ・最大 0.42 秒）。
- `data-reveal-stagger` は**トラック側**に付けた。カード1枚ずつに付けると、
  横スクロールで画面外におるカードがいつまでも浮上せんようになるけん。

### Added
- `ayn-sleep-radio/scripts/gen_world_assets.py` — 背景世界の水彩パーツを
  ChatGPT プラグインで生成する。既存の `gpt_art_poster` の仕組みに乗せとる
  （同じ会話で2通目以降も送れる `single_chat` 対応済み）。
  - 白背景の抜きは **rembg やなく輝度→アルファ**。硬く切り抜くと水彩のにじみが
    死ぬけん、明るいところほど透明になる連続的な変換にして、抜いたぶん色を戻す
    （unpremultiply）。
  - 出力 `public/world/<name>.png`（透過）。生画像は `_raw/` に残す（git 管理外）。

### 進捗
- 1枚目 `wave_mid` 実機成功（2243×701）。プロンプトの型が効くのを確認:
  「横長の帯」「下半分だけに置く」「背景は真っ白な紙のまま」「文字/UI/人物は描かん」。
- 残り6枚（wave_far / wave_near / cloud_loop / ship_hull / ship_sail / ship_lantern）生成中。

## 2026-09-25 (2) — 空に時間を流す（夕べ 〜 夜の連続航海）

「みさとと」を解剖した結果を受けた作り直しの第1段。

### 調べたこと（みさとと の構造）
- `.Map` が `position: fixed` の **canvas 1枚**。ページ高 28,800px＝36画面ぶん。
- 背景は約60枚の **PNG部品**を canvas で合成しとる。`<img>` は8枚だけ（サムネ）。
- 効いとるのは3つ:
  1. **1枚絵やなく部品に割る** — `kagura_head` / `kagura_hand_left` / `ayu_body` のように
     頭・手・体が別ファイル。別々に揺らせるけん生きて見える
  2. **ループタイル** — `cloud_loop` / `cloud_loop_top` / `people_loop13` で帯を無限に流す
  3. **固定canvasをカメラで動かす** — スクロール＝巨大な絵の中のカメラ位置

### Changed
- 空を **スクロールで進む時間**にした。既存の `<html data-depth="0..5">`
  （寄港地）にそのまま乗せて、港を越えるたび 1.6 秒かけて移る:
  夕べ(港を出る) → 宵の口 → 藍に沈む → 沖の夜 → 深夜の霧 → 夜明け前。
  「気づいたら日が暮れとった」くらいの速さ。`prefers-reduced-motion` では即時。

### 次にやること
- 水彩PNG部品を GPT プラグインで生成 → 部品合成エンジンへ流し込む。
  いまの procedural な波・粒子は、部品が揃うまでの下地として残す。

## 2026-09-25 — 背景を「情報の海」にする（スクロール連動）

参考にした3サイトの型は **固定した一枚の世界がスクロールで姿を変える**。
それを BCNOFNe に落として、地の色ごと夜空にした。

### Added
- `src/components/OceanField.astro` — 背景の世界。canvas 2D 1枚に4つ重ねて描く:
  1. **島影** … スクロールで右から左へ流れる（＝渡っとる証拠）
  2. **うねり** … 薄いバンド3枚。位相が送られて波が動く
  3. **小舟** … 波に乗って上下し、ランタンが灯っとる
  4. **粒子** … データの粒。星みたいに漂って、近いもの同士が細い線でつながる（星座）
- 背景コラージュに**視差**。写真を奥／手前の2束に分けて、奥はゆっくり手前は速く流す。

### Changed
- **ページの地を夜空に**（`html` の固定グラデ）。以降このサイトは常に夜の海。
- コラージュのベールを「白で飛ばす」→「夜に沈める scrim」へ。
  写真が“夜の海に浮かぶ思い出”に見える濃さにした。
- セクションの紙だまりを濃く（0.72 → 0.93）。夜空の上でも本文が白い紙に乗る。

### 設計メモ
- **空はキャンバスで塗らん**。一度 canvas に空を描いたら、手前の水彩コラージュが
  全部隠れて失敗した。空は CSS の地、星と海だけキャンバス、で両立した。
- キャンバスは写真コラージュと**同じ z:-1 で DOM の後ろ**に置く＝写真の上・本文の下。
  本文の所は紙だまりが更に上に乗るけん、海は自然と**港と港の間で濃く見える**。
- 濃さの調整は OceanField の `DAY`/`NIGHT` と CollageBackdrop の
  `.pagecollage__item { opacity }` の2か所だけ。

### 性能
- canvas 1枚・**30fps に間引き**・粒の光は4色スプライトに事前に焼いて drawImage
  （毎フレームの `createRadialGradient` は高い）。
- **スクロールイベントは張らん**（毎フレーム `scrollY` を読むだけ）。視差も同じループ。
- タブが隠れたら描かん。`prefers-reduced-motion` / `save-data` は**1枚だけ描いて終わり**。
- 最初の1枚は rAF を待たずに描く（裏タブ等で rAF が来ん環境がある＝前に踏んだ穴）。

## 2026-09-25 — 機関室の予告欄をスクロール窓に

- トップの「機関室」予告を **新しい 3 本固定 → 新しい 30 本までを窓の中でスクロール**に。窓の高さは約 5 本ぶん（`LOG_WINDOW_ROWS`）、5 本以下の間はスクロール無しで今までどおり。
- スマホで窓の端まで来てもページごと流れ出さん（`overscroll-behavior: contain`）。下端のうっすら霧＋「↕ 窓の中をスクロール…」の一言で続きがあると分かる。最後まで読んだら霧は消える。キーボードでもスクロール可（tabindex）。
- 30 本を超えたらボタンが「機関室に入る（ぜんぶの日誌）」になる。/engine-room/ はページ全体スクロールのまま。
- 確認: 仮の日誌 12 本を一時的に置いて 375px 幅で表示・スクロール・横はみ出し無しを確認（仮の分は削除済み）。
